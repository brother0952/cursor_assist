...
def test_search_options():
    ...
    try:
        ...
        # 显示前5个房价相关链接
        if house_price_links:
            logger.info("前5个房价相关链接:")
            for i, link_text in enumerate(house_price_links[:5]):
                logger.info(f"  {i+1}. {link_text}")
        
        # 等待用户观察
        logger.info("等待15秒后关闭浏览器...")
        time.sleep(15)
        
        # 关闭浏览器
        driver.quit()
        
        return stats_success or title_success
