...
class CNN(nn.Module):
    def __init__(self):
        ...
        # 第二个卷积层，10个输入通道，20个输出通道，3x3卷积核
        self.conv2 = nn.Conv2d(10, 20, kernel_size=3, padding=0)
        # 全连接层
        self.fc1 = nn.Linear(10 * 10 * 20, 500)
    ...
    def forward(self, x):
        ...
        x = F.max_pool2d(x, 2 , stride=2) # 最大池化层，2x2的窗口，步幅为2 ,28->24->12
        # 第二个卷积层 + ReLU + 最大池化
...
def main():
    ...
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.1307,), (0.3081,))  # MNIST数据集的均值和标准差. 正则化作用，防止过拟合，降低模型复杂度
    ])
